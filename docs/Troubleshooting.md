...
| `docker pull` works but Buildx reports an `x509` error at `/service/token` | The Buildx client needs the CA as well as the Docker daemon. On the login node, prefix the build command with `SSL_CERT_DIR=/etc/docker/certs.d/harbor.cvgl.lab`; see the [verified Buildx setup](Buildx_and_Harbor.md). |
