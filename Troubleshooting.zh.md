...
| HTTPS 报告未知证书颁发机构 | 通过管理员获取并核验集群 CA，然后为实际客户端配置证书信任。浏览器、Python、Docker daemon 和独立 BuildKit builder 可能需要分别设置。 |
| `docker pull` 正常，但构建报告 `x509` 错误 | 用 `docker buildx ls` 确认 builder；按 [Harbor 兼容流程](Custom_Containerized_Environment.zh.md)处理。在替代方案完成核验前，继续使用文档中的 `DOCKER_BUILDKIT=0` 路径。 |
...
原生 `det` 支持的 CA 设置是 `DET_MASTER_CERT_FILE`。对于 `determined-compute`，启用 `--verify-ssl` 并配置 Python Requests 信任，例如让 `REQUESTS_CA_BUNDLE` 指向已核验的 CA bundle。应在实际发出请求的进程环境中设置这些变量。禁用校验不能修复证书信任。
